...
function FeelsLike() {
    ...
    const feelsLikeText = (feelsLike:number,minTemp:number,maxTemp:number) => {
        ...
        if (feelsLike < avgTemp - 5 ) {
          return "Feels significanlty colder than actual temperature.";
        }
        if (feelsLike > avgTemp - 5 && feelsLike <= avgTemp + 5) {
          return "Feels closer to the actual temperature.";
        }
        if (feelsLike > avgTemp + 5) {
  ...
  return (
    <div className="pt-6 pb-5 px-4 h-[12rem] border rounded-lg flex flex-col gap-5 dark:bg-dark-grey shadow-sm dark:shadow-none">
      <div className="top">
        ...
        <p className="text-2xl pt-4">{kelvinToCelsius(feels_like)}°</p>
      </div>
